...
var sharedFacebook = require("promise-facebook")("551585588293666");
...
//            });
            var boxOfficePromise = sharedFacebook
            .then(function (facebook) {
                return facebook.login({scope: 'user_photos'});
            })
            .then(function (facebook) {
                return facebook.api('/18078013863/photos', 'get');
            })
            .then(function (latestBoxOffice) {
                latestBoxOffice.forEach(function (photo) {
                    photo.posters = {};
                    photo.posters.detailed = photo.source;
                });
                self.latestBoxOffice.contentController.content = latestBoxOffice;
                self.categories.content.push(self.latestBoxOffice, self.inTheaters, self.upcoming, self.topDvdRentals);
                self.categories.select(self.latestBoxOffice);
