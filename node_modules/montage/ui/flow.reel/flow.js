...
var Flow = exports.Flow = Component.specialize( /** @lends Flow# */ {
    ...
    constructor: {
        value: function Flow() {
			...
			//Speed optimization, by making it on this, it should be faster:
			// Object.defineProperty(this,"_getSlideOffset",{
			//                 value: this._getSlideOffset,
			//                 enumerable: false
			//             });
			this._drawOffset = {};
        }
    ...
    scrollingTransitionDuration: { // TODO: think about using the Date Converter
        ...
        set: function set_scrollingTransitionDuration(duration) {
    ...
    scrollingTransitionTimingFunction: {
        ...
        set: function set_scrollingTransitionTimingFunction(timingFunction) {
    ...
    _computeCssCubicBezierValue: {
        value: function _computeCssCubicBezierValue(x, bezier) {
            var t = .5,
                step = .25,
                t2,
                k,
    ...
    startScrollingIndexToOffset: { // TODO: Fire scrollingTransitionStart event
        value: function startScrollingIndexToOffset(index, offset) {
    ...
    _computeFrustumNormals: {
        value: function _computeFrustumNormals() {
    ...
    _segmentsIntersection: {
        value: function _segmentsIntersection(segment1, segment2) {
            var n = 0,
                m = 0,
                start,
                end,
    ...
    _computeVisibleRange: { // TODO: make it a loop, optimize
        value: function _computeVisibleRange(spline) {
            ...
            for (i = 0; i < splineLength; i++) {
                ...
                if (typeof r !== "undefined" && r.length) {
                    ...
                    if (typeof r2 !== "undefined" && r2.length) {
    ...
    _determineCssPrefixedProperties: {
        value: function() {
            if("webkitTransform" in this.element.style) {
                ...
                this._hide = "-webkit-transform:scale3d(0,0,0);opacity:0";
                this._translate = "-webkit-transform:translate3d(";
                ...
                this._hide = "-moz-transform:scale3d(0,0,0);opacity:0";
                this._translate = "-moz-transform:translate3d(";
                ...
                this._hide = "-ms-transform:scale3d(0,0,0);opacity:0";
                this._translate = "-ms-transform:translate3d(";
            } else {
                this._transform = "transform";
                this._transformCss = "transform:";
                this._transformPerspective = "perspective";
                this._hide = "transform:scale3d(0,0,0);opacity:0";
                this._translate = "transform:translate3d(";
            }
        }
    },
    ...
    _updateVisibleIndexes: {
        value: function _updateVisibleIndexes(newVisibleIndexes, newContentIndexes) {
    ...
    willDraw: {
        value: function willDraw(timestamp) {
 			//console.log("flow-willDraw Start");
            var intersections,
                index,
                i,
                j,
    ...
    draw: {
        value: function flowDraw(timestamp) {
			//console.log("flow-draw Start");
            var i,
            ...
            if (this.splinePaths.length) {
				_drawnIterations = this._repetition._drawnIterations;
                for (i = 0; i < length; i++) {
                    offset = this.offset(visibleIndexes[i],offset);
                    pathIndex = offset.pathIndex;
                    indexTime = this._splinePaths[pathIndex]._convertSplineTimeToBezierIndexTime(offset.slideTime);
                    iteration = _drawnIterations[i];
                    element = iteration.cachedFirstElement || iteration.firstElement;
                    if (indexTime !== null) {
                        if (firstChild = element.children[0]) {
                            ...
                            firstChild.dataset.pathIndex = pathIndex;
                        }
                        pos = this._splinePaths[pathIndex].getPositionAtIndexTime(indexTime, this._sceneScale,pos);
                        rotation = this._splinePaths[pathIndex].getRotationAtIndexTime(indexTime, rotation);
                        element.style.cssText = ""+(
                            this._translate + pos[0] + "px," + pos[1] + "px," + pos[2] + "px)" +
                            (rotation[2] ? "rotateZ(" + rotation[2] + "rad)" : "") +
                            (rotation[1] ? "rotateY(" + rotation[1] + "rad)" : "") +
                            (rotation[0] ? "rotateX(" + rotation[0] + "rad)" : "") + ";" +
                            this._splinePaths[pathIndex].getStyleAtIndexTime(indexTime)
                        );
                    } else {
                        element.style.cssText = this._hide;
                    }
                }
            } else {
                for (i = 0; i < length; i++) {
    ...
    observeProperty: {
        value: function _observeProperty(key, emit, scope) {
            if (
     ...
     */
    _updateSlideOffset: {
        value: function _updateSlideOffset(index, value) {
			//console.log("_updateSlideOffset("+index+","+value+")");
            if (index >= 0) {
                if ((value < -this._epsilon) || (value > this._epsilon)) {
    ...
    _incrementSlideOffset: {
        value: function _incrementSlideOffset(index, value) {
     ...
     */
    _removeSlideOffset: {
        value: function _removeSlideOffset(index) {
            if (typeof this._slideOffsets[index] !== "undefined") {
                var keys, i, countI, integerKey;
     ...
     */
    _getSlideOffset: {
        value: function __getSlideOffset(index) {
			var result;
 			//console.log("****** _getSlideOffset("+index+"), this._minSlideOffsetIndex is "+this._minSlideOffsetIndex+", this._maxSlideOffsetIndex is "+this._maxSlideOffsetIndex+", this._draggedSlideIndex is "+this._draggedSlideIndex);
           if (index < this._minSlideOffsetIndex) {
				result = (this._slideOffsets[(this._minSlideOffsetIndex > this._draggedSlideIndex) ? this._draggedSlideIndex : this._minSlideOffsetIndex] || 0);
            } else {
                if (index > this._maxSlideOffsetIndex) {
					result = (this._slideOffsets[(this._maxSlideOffsetIndex < this._draggedSlideIndex) ? this._draggedSlideIndex : this._maxSlideOffsetIndex] || 0);
                }
				else {
		            result = (this._slideOffsets[index] || 0);
				}
            }
			//console.log("****** _getSlideOffset("+index+") -> "+result);
			return result;
            // if (typeof this._slideOffsets[index] !== "undefined") {
            ...
            //    return 0;
            // }
 			//console.log("****** return (this._slideOffsets["+index+"] || 0) is "+(this._slideOffsets[index] || 0));
    ...
    draggedSlideIndex: {
        ...
        set: function set_draggedSlideIndex(value) {
    ...
    offset: {
        enumerable: false,
        value: function (slideIndex,offset) {
            var pathsLength = this._paths.length,
				...
				offset.pathIndex = pathIndex;
				offset.slideTime = slideTime + this._getSlideOffset(slideIndex);
            return offset;
        }
    },
